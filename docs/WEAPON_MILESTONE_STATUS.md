...
- GPL-attributed Superb Warfare presentation architecture: synthetic camera/root hierarchy, continuous ADS/walk/sprint/draw motion, nonlinear recoil curves, player-skin arms attached to rotated model bones, mechanism motion, and model-local muzzle flare.
- HBM-authored presentation rigs for all four pilots. The failed Target Pistol profile and its near-camera world-space flash were removed; the replacement build still requires in-game visual acceptance.
...
| Ballistics tick budget | pass | The 16 StG 77 plus 8 SPAS-12 profile averaged `0.1293941463414634 ms` across 205 measured ballistics ticks in the replacement build, below the `5 ms` gate. Profiling is dormant outside an explicitly reset test window. |
...
| Package contamination audit | pass | The replacement `hbm-0.1.0-alpha.jar` contains 12,349 entries and no Superb Warfare, SimpleBedrockModel, or `META-INF/jarjar` paths. |
...
- Inspect the rebuilt Target Pistol in first person and third person across hip-fire, ADS, firing, dry-fire, empty slide lock, sprint, and staged reload. Confirm that the gun is forward-facing, both arms remain attached, and no local world-space flame or smoke crosses the camera. Then perform the same checks for StG 77, SPAS-12, and Congo Lake plus GUI, dropped-form, FOV-extreme, and shader-loader checks. Static bone validation cannot prove transform quality.
