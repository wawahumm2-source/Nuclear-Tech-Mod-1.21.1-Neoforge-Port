...
| Ballistics tick budget | pass | The 16 StG 77 plus 8 SPAS-12 profile averaged `0.11649707317073171 ms` across 205 measured ballistics ticks, below the `5 ms` gate. Profiling is dormant outside an explicitly reset test window. |
...
- Inspect every pilot in first person, third person, GUI, dropped form, ADS, sprint, reload, empty state, both FOV extremes, and a shader-loader environment. Static bone validation cannot prove transform quality.
