...
| Firearm framework | modern wrapper | mapped | exact/mapped HBM | n/a | approved divergence | Firearms intentionally use the server-authoritative modern framework recorded in `FIREARMS_PARITY_DIVERGENCE.md`; later parity audits must preserve HBM identity but must not restore legacy handling. The four-gun slice remains gated on runtime, multiplayer, visual, and performance acceptance. |
| Pilot ammunition progression | mapped | exact/mapped HBM | n/a | n/a | temporary scaffold | Eight pilot loads are survival-craftable through Burner Press components and shaped assembly. Direct steel and ammunition-stamp recipes are temporary access paths until the corresponding HBM material and assembly-machine progression exists. |
