...
| Target Pistol | `src/main/resources/assets/hbm/animations/weapon/star_f.animation.json` | `c1765f9342dcbf99a2e460197a9b97d779b87c0ac2172fa9c5e1a8df9793420a` | GeckoLib `1.8.0`; 11 framework clips plus coordinated `reload_normal` and `reload_empty` presentation clips. The independently authored MP-443-style motion coordinates `Gun`, `Slide`, `Hammer`, `Mag`, `camera`, and both player-skin hand anchors without redistributing Superb Warfare artwork. |
...
No lossy `*.geo.json` duplicate exists. `ObjBakedGeoModelLoader` (`5b4fd3c895ff45a05fb29d9e976d78a587aeba3d94c09eb7b35a7f46b0b08ff7`) reads the original OBJ vertices, UVs, normals, and faces into a cached GeckoLib `BakedGeoModel`; each OBJ object/group becomes an animation bone. It surrounds that mechanism with synthetic `camera` and `root` bones and adds empty `Righthand`, `Lefthand`, and `flare` bones from `SuperbGunRig` (`db641b033f502f6688ad0639373d132c47ee05dbc8c2cd2a157fc9e1790cf569`). These bones contain no copied geometry or texture. `HbmGunGeoRenderer` (`d6e98a35f8dbf3d36000405213412ec450a2180f8a6c122ef4d02df18dacb62b`) cancels GeckoLib's conventional half-block item-centering offset in first person because the OBJ bridge already emits centered coordinates. `HbmMuzzleFlashRenderer` (`b3cf82dea62bf3e9dc0750503a0fa7e0d6f45a38f3223d0bcdcb18581e8883af`) explicitly translates HBM's authored muzzle coordinate into the same `(-x, y, z) / 16` model space and uses bounded effect sizes. `SuperbGunPresentationState` (`ce96e9eebca5860b0fd423b207c1f66b915c8220b999d6f8eea76ab61ca2a193`) implements the attributed continuous motion, ADS easing, nonlinear recoil curves, and dynamic crosshair inputs. `ClientWeaponController` (`5b2ae712a8ffe5f4eb0a845ad48f8bd401d810df95b92f88a454dd1a2dd23868`) replaces the vanilla crosshair while a gun is held and draws a source-guided HUD with HBM/vanilla primitives and item icons. All four animation documents provide the 11 required states (`equip`, `idle`, `ads`, `fire`, `dry_fire`, `reload_start`, `reload_loop`, `reload_end`, `inspect`, `sprint`, and `lower`) with nonempty mechanism motion. The validator confirms every animated bone exists in its source OBJ or in the explicitly synthesized viewmodel contract. HBM rig coordinates and final visual parity still require in-game inspection.
