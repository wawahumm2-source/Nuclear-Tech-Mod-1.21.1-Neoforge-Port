...
| Target Pistol | `src/main/resources/assets/hbm/animations/weapon/star_f.animation.json` | `a23849d4ed06866f50839c34295104ee9c7dc95d55b8737daf9238952cf72b64` | GeckoLib `1.8.0`; 11 framework clips plus coordinated `reload_normal` and `reload_empty` presentation clips. The independently authored MP-443-style motion keeps root recoil procedural, constrains the magazine and player-skin hand paths outside the near-camera plane, and animates `Slide`, `Hammer`, and `Mag` without redistributing Superb Warfare artwork. |
...
No lossy `*.geo.json` duplicate exists. `ObjBakedGeoModelLoader` (`5b4fd3c895ff45a05fb29d9e976d78a587aeba3d94c09eb7b35a7f46b0b08ff7`) reads the original OBJ vertices, UVs, normals, and faces into a cached GeckoLib `BakedGeoModel`; each OBJ object/group becomes an animation bone. It surrounds that mechanism with synthetic `camera` and `root` bones and adds empty `Righthand`, `Lefthand`, and `flare` bones from `SuperbGunRig` (`6396f5eb303bb19974199e5964fdd19654516e947076d4109dfee4a70cd04313`). These bones contain no copied geometry or texture. `HbmGunGeoRenderer` (`aaf93811e97db875f3ae82e160bb1b5e67b903e5f81734feddf1f6a74b554b25`) preserves the OBJ bridge's centered coordinates and dispatches per-rig player-arm scale plus model-local muzzle effects. `HbmMuzzleFlashRenderer` (`b3cf82dea62bf3e9dc0750503a0fa7e0d6f45a38f3223d0bcdcb18581e8883af`) explicitly translates HBM's authored muzzle coordinate into the same `(-x, y, z) / 16` model space and uses bounded effect sizes. `SuperbGunPresentationState` (`e793129b7fbde50e1b5f7984e2dbfd53a8812a6ee1f6bc10ccf455d5aa61ba93`) implements the attributed continuous motion, ADS easing, nonlinear recoil curves, monotonic sprint transition, and dynamic crosshair inputs. `ClientWeaponController` (`224a325ea46c14a271d7cf5fb84d9030c692fcacdb2a4985fdd943ebd5c45f4d`) prevents Target Pistol pose clips from double-transforming the procedural root, replaces the vanilla crosshair while a gun is held, and draws a source-guided HUD with HBM/vanilla primitives and item icons. All four animation documents provide the 11 required states (`equip`, `idle`, `ads`, `fire`, `dry_fire`, `reload_start`, `reload_loop`, `reload_end`, `inspect`, `sprint`, and `lower`) with nonempty mechanism motion. The validator confirms every animated bone exists in its source OBJ or in the explicitly synthesized viewmodel contract. HBM rig coordinates and final visual parity still require in-game inspection.
