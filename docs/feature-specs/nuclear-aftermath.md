...
- NTM Extended 3.0.3 `EntityNukeTorex` and `RenderTorex`: use the exact `particle_base.png` cloudlet and `flare.png` assets, radius-scaled flash and flare, a shock front advancing at two blocks per tick, source hurt-camera feedback at arrival, an uncapped rising core, torus growth, humidity-driven condensation rings, and lifetime `45 * 20 * clamp(radius * 0.01, 0.25, 5)` ticks.
...
- The active client cloud follows NTM Extended 3.0.3's dynamic Torex presentation. Standard cloudlets rise from a terrain-following emitter into the stem, circulate through a growing toroidal cap, transition from the source hot yellow-orange palette to brown-gray, and remain in motion until the source simulation speed reaches zero at removal. Early source shock and ring cloudlets and biome-downfall-driven condensation rings use the same lifecycle.
- Little Boy radius `120` produces the source float scale near `1.2`, lasts `1079` ticks (approximately 54 seconds), and fades during its final quarter. This client-only lifetime does not extend fallout processing, damage, fire, terrain work, or server jobs.
...
- WMD `0.1.2` was inspected only as a visual benchmark. Its CurseForge project is All Rights Reserved, so this port does not copy its code, compute shaders, textures, sounds, or particle data. The active cloud uses the selected GPL-3.0 NTM Extended 3.0.3 source and assets instead.
